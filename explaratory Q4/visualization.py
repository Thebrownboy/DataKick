...
def plot_bar_SOT(df_copy):
    avg_SOT = df_copy.groupby('Result')['SOT'].mean()
    plt.figure(figsize=(10, 10))
    plt.bar(avg_SOT.index, avg_SOT.values, color=[
            '#FF8C00', '#228B22', '#4169E1'])
    plt.title('Average shots on target for Result Categories')
    plt.xlabel('Result')
    ...
    plt.grid(axis='y', linestyle='--', alpha=0.5)
    plt.gca().set_facecolor('#F5F5F5')
    for i, v in enumerate(avg_SOT.values):
        plt.text(i, v + 1, str(round(v, 2)), ha='center',
                fontsize=12, fontweight='bold')
        
    plt.savefig('./output plots/avg_SOT.png')
    

# df_augmented = increase_data(df_copy)
plot_bar_poss(df_copy)
plot_relation_res_pos(df_copy)
plot_relation_res_SOT(df_copy)
plot_bar_SOT(df_copy)
